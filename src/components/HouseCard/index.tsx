...
const HouseProperties = ({ properties }: { properties: HouseProperty[] }) => {
  ...
  const icons = properties.map(({ description, value }) => {
    const key = useId()
    return (
...
export const HouseCard = ({ id, name, description, price, properties }: House) => {
  return (
    <div
      key={id}
      className='max-w-[18rem] bg-gray-200 rounded-md shadow-black/50 shadow-md'>
